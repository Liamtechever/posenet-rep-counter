...
const universalState = {
  ...
  function updateUniversalState(pose, previousPose) {
    // --- Posture Detection ---
    // For example, use knee angles to decide if the user is squatting or standing.
    ...
    // If both knees are bent significantly (angles less than a threshold), assume squatting.
    ...
    // If the nose is very low (e.g., near the bottom of the canvas), assume lying down.
    // (Assume canvas height is around 500; adjust as needed.)
    ...
    // --- Movement Detection (using previous pose if provided) ---
    if (previousPose) {
      // Example: detect arm swing on the right side
      ...
      if (prevRightWrist && currRightWrist &&
        ...
        universalState.movement.armSwingRight = dx > 20; // threshold (adjust as needed)
      }
      // Example: detect arm swing on the left side
      ...
      // Example: detect walking toward the camera by comparing shoulder distances
